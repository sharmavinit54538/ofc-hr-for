...
function PayrollPoliciesPage() {
  const { data: policyRes, isLoading } = useGetPoliciesByCategoryQuery("Payroll", {
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="glass-tile rounded-2xl p-12 text-center text-xs text-muted-foreground flex flex-col items-center justify-center gap-2">
          ...
          Loading Payroll policies...
        </div>
      ) : policies.length === 0 ? (
        <div className="glass-tile rounded-2xl p-12 text-center text-xs text-muted-foreground flex flex-col items-center justify-center gap-2">
          <Inbox className="size-8 text-muted-foreground/50" />
          <p className="font-medium text-foreground text-sm">No Payroll Policies Published</p>
        </div>
      ) : (
