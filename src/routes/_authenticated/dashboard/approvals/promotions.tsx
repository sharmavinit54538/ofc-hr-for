...
function PromotionsApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Promotions" });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Promotion & Band Elevation Approvals"
        description="Merit-based promotion requests, band hikes, and role elevation approvals."
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No promotion approval requests</p>
          <p className="text-xs text-muted-foreground/60">Promotion requests will appear here when submitted.</p>
        </div>
      ) : (
