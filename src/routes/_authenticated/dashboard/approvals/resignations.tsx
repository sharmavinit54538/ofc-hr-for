...
function ResignationsApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Resignations" });
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      toast.success("Resignation Request Processed");
    } catch {
      toast.error("Failed to process resignation request.");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Resignation & Exit Approvals"
        description="Employee resignation acceptance, notice period waivers, and exit clearance approvals."
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No resignation approval requests</p>
          <p className="text-xs text-muted-foreground/60">Resignation requests will appear here when submitted.</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-border/60">
                {items.map((i) => (
                  <tr key={i.id} className="transition-colors hover:bg-secondary/40">
                    ...
                    <td className="px-5 py-4 font-mono text-muted-foreground">{i.submittedDate}</td>
