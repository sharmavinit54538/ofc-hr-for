...
function OvertimeApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Overtime" });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Overtime & Extra Hours Approvals"
        description="Weekend, holiday, and after-hours overtime payout approval queue."
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No overtime approval requests</p>
          <p className="text-xs text-muted-foreground/60">Overtime requests will appear here when submitted.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left text-xs">
              <thead className="border-b border-border/60 bg-card/60 uppercase tracking-[0.08em] text-muted-foreground">
                <tr>
                  ...
                  <th className="px-5 py-3.5 font-bold">Hours / Amount</th>
