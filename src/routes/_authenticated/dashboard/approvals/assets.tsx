...
function AssetsApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Assets" });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No asset approval requests</p>
          <p className="text-xs text-muted-foreground/60">Asset requisition requests will appear here when submitted.</p>
        </div>
      ) : (
