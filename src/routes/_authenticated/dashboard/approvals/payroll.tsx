...
function PayrollApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Payroll" });
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      toast.success("Payroll Request Approved");
    } catch {
      toast.error("Failed to approve payroll request.");
    }
  };
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No payroll approval requests</p>
          <p className="text-xs text-muted-foreground/60">Payroll approval requests will appear here when submitted.</p>
        </div>
      ) : (
