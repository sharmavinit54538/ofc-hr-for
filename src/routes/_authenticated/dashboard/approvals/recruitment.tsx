...
function RecruitmentApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Recruitment" });
  ...
  const handleApprove = async (id: string) => {
    try {
      ...
      toast.success("Recruitment Request Approved");
    } catch {
      toast.error("Failed to approve recruitment request.");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Recruitment & Hiring Approvals"
        description="Job requisition, offer letter, and candidate selection approval queue."
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No recruitment approval requests</p>
          <p className="text-xs text-muted-foreground/60">Recruitment requests will appear here when submitted.</p>
        </div>
      ) : (
