...
function TransfersApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Transfers" });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Employee Transfer & Relocation Approvals"
        description="Inter-department and inter-location transfer approvals and relocation packages."
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No transfer approval requests</p>
          <p className="text-xs text-muted-foreground/60">Transfer requests will appear here when submitted.</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-border/60">
                {items.map((i) => (
                  <tr key={i.id} className="transition-colors hover:bg-secondary/40">
                    ...
                    <td className="px-5 py-4 text-muted-foreground">{i.requesterDept}</td>
