...
function AttendanceApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Attendance" });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No attendance approval requests</p>
          <p className="text-xs text-muted-foreground/60">Attendance regularization requests will appear here when submitted.</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-border/60">
                {items.map((r) => (
                  <tr key={r.id} className="transition-colors hover:bg-secondary/40">
                    <td className="px-5 py-4 font-mono font-bold text-primary">{r.approvalId}</td>
                    <td className="px-5 py-4 font-bold text-foreground">{r.requestTitle}</td>
                    <td className="px-5 py-4 text-muted-foreground">{r.requesterName}</td>
                    <td className="px-5 py-4 font-mono text-muted-foreground">{r.submittedDate}</td>
                    <td className="px-5 py-4 text-right">
                      {r.status === "Pending" ? (
                        <button onClick={() => handleApprove(r.id)} className="rounded-lg bg-emerald-500/10 px-2.5 py-1 text-xs font-bold text-emerald-400 border border-emerald-500/20">
                          <ThumbsUp className="inline size-3 mr-1" />Approve
                        </button>
                      ) : (
                        <span className="text-muted-foreground font-mono text-[11px]">{r.status}</span>
                      )}
                    </td>
                  </tr>
                ))}
