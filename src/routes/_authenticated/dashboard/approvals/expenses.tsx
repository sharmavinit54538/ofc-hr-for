...
function ExpensesApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Expenses" });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No expense approval requests</p>
          <p className="text-xs text-muted-foreground/60">Expense claims will appear here when submitted.</p>
        </div>
      ) : (
