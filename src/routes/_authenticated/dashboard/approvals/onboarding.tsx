...
function OnboardingApprovalsPage() {
  const { data: res, isLoading } = useGetApprovalsQuery({ type: "Onboarding" });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="New hire onboarding document verification and access provisioning approvals."
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No onboarding approval requests</p>
          <p className="text-xs text-muted-foreground/60">Onboarding requests will appear here when submitted.</p>
        </div>
      ) : (
