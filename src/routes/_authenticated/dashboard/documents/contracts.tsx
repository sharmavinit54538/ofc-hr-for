...
function ContractsPage() {
  const { data: res, isLoading } = useGetDocumentsQuery({ category: "Employment Contract" });
  const docs = res?.data ?? [];
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-xs text-muted-foreground/60">Employment contracts will appear here when uploaded.</p>
        </div>
      ) : (
