...
function EmployeeDocumentsPage() {
  const { data: res, isLoading } = useGetDocumentsQuery({ category: "Employee Document" });
  const docs = res?.data ?? [];
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="glass-tile rounded-2xl flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">No employee documents found</p>
          <p className="text-xs text-muted-foreground/60">Employee documents will appear here when uploaded.</p>
        </div>
      ) : (
