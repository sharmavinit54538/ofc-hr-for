import { createFileRoute } from "@tanstack/react-router";
import { PageHeader } from "@/components/admin/page-header";
import { ModuleCard } from "@/components/admin/module-card";
import { SIDEBAR_NAV_ITEMS } from "@/lib/admin-navigation";

export const Route = createFileRoute("/_authenticated/dashboard/payroll/")({
  component: PayrollLandingPage,
});

function PayrollLandingPage() {
  const nav = SIDEBAR_NAV_ITEMS.find((item) => item.id === "payroll");

  return (
    <div className="space-y-8">
      <PageHeader
        title="Payroll & Compensation Management"
        description="Salary components, monthly pay run execution, statutory tax compliance, TDS/PF filings, and digital payslip distribution."
        breadcrumbs={[{ label: "Payroll" }]}
      />

      <div className="space-y-4">
        <h2 className="font-display text-lg font-bold text-foreground">
          Payroll Modules
        </h2>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-2">
          {nav?.subModules.map((subModule) => (
            <ModuleCard key={subModule.id} module={subModule} />
          ))}
        </div>
      </div>
    </div>
  );
}
